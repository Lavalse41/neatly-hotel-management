...
const validateRegistrationData = (req, res, next) => {
    const { fullName, password, email, birth_day, country, idNumber, card_owner, } = req.body;
    ...
    if (names.length !== 2 ||
        ...
        !/^[a-zA-Z]*$/.test(names[1])) {
        return res.status(400).json({
            error: "Invalid full name. It should include both the first name and last name without numbers.",
        });
    }
    ...
    if (cardNames.length !== 2 ||
        ...
        return res.status(400).json({
            error: "Invalid card name. It should include both the first name and last name without numbers.",
        });
    }
...
authRouter.post("/register", avatarUpload, validateRegistrationData, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    let avatarUrl = null;
    ...
    const encryptKey = Buffer.from("00112233445566778899aabbccddeeff00112233445566778899aabbccddeeff", "hex");
    ...
    const { data: data, error: dataError } = yield supabase
        .from("credit_card")
        .insert([
        {
    ...
    let { data: creditCard, error: errorCreditCard } = yield supabase
        .from("credit_card")
    ...
    const { data: updateCard, error: updateCardError } = yield supabase
        .from("users")
...
authRouter.post("/login", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const token = jwt.sign({ email: user.email, user_id: user.id }, `${process.env.SECRET_KEY}`, {
        expiresIn: "900000",
    });
    const userData = user;
